...
import { PDFDocument, rgb } from "../vendor/pdf-lib.esm.min.js?v=49";
import { getTrimSizeById } from "../modules/canvasEngine.js?v=49";
import { computeCanvasDimensions } from "../modules/bleedEngine.js?v=49";
import { computeSafeZone } from "../modules/safeZoneEngine.js?v=49";
import { getSizesForSelection, buildCombinedPalette, applyCustomColorOrder } from "../modules/colorKeyEngine.js?v=49";
import { computePagination } from "../modules/storyboardEngine.js?v=49";
import { isPageEnabled, computeFrontMatterPageCount, orderedFrontMatterPages, orderedBackMatterPages } from "../modules/frontBackMatterEngine.js?v=49";
import { buildSolutionPages } from "../modules/solutionGenerationEngine.js?v=49";
import { BORDER_PRESETS } from "../modules/borderStyleEngine.js?v=49";
import { migratedKeyStyle } from "../modules/layoutEngine.js?v=49";
import { resolveEffectiveGrid } from "../modules/resolutionScalingEngine.js?v=49";
import { computeKeyGridLayout, keyEntryPosition } from "../modules/colorKeyLayoutEngine.js?v=49";
import { normalizeComposition, computeLayout } from "../modules/layoutCompositionEngine.js?v=49";
import { resolveActiveAsset } from "../modules/assetGalleryEngine.js?v=49";
import { renderFullMosaicGrid, getPlaceholderSource, loadImageSource, drawSourceToCanvas } from "./mosaicRenderer.js?v=49";
import { isContentPageBlack, isFacingPageBlack, isBlackWhiteEdition, toGrayscaleRgb } from "../modules/bookThemeEngine.js?v=49";
import { getFontById, fontAssetUrl } from "../modules/fontLibraryEngine.js?v=49";
import { applySourceSmoothing } from "../modules/sourceSmoothingEngine.js?v=49";
...
async function resolveItemSource(item, smoothingMode, maxSize = 1024) {
  try {
    const img = await loadImageSource(item.objectUrl);
    return applySourceSmoothing(drawSourceToCanvas(img, maxSize), smoothingMode);
  } catch {
...
function resolveItemEffectiveSettings(item, state, globalPalette) {
  ...
  const basePalette = item.settings.colorSetOverride ? buildCombinedPalette([item.settings.colorSetOverride]) : globalPalette;
  const palette = applyCustomColorOrder(basePalette, item.settings.customColorOrder);

  const cornerTrimCorners = item.settings.cornerTrimCorners ?? state.gridCornerTrimCorners;
  ...
  const sourceResolution = item.settings.sourceResolution ?? state.sourceResolution;

  return { gridPattern, borderWeightPt, gridTintPercent, cornerRadiusPercent, palette, cornerTrimCorners, cornerTrimShape, cornerTrimSizePercent, sourceSmoothing, sourceResolution };
}
...
function drawKeyEntries(page, rect, palette, font, textColor, blackWhiteEdition, orientation, align = "start", maxPerLine = null) {
  ...
  if (blackWhiteEdition) {
    drawKeyEntriesBlackWhite(page, rect, palette, font, textColor, orientation, align, maxPerLine);
  } else {
    drawKeyEntriesColor(page, rect, palette, font, textColor, orientation, align, maxPerLine);
  }
}
...
function drawKeyEntriesBlackWhite(page, rect, palette, font, textColor, orientation, align, maxPerLine) {
  const { xPt: xPtBase, yPt, widthPt, heightPt } = rect;
  const { cols, rows, entryWidthIn, entryHeightIn } = computeKeyGridLayout(palette.length, widthPt / PT_PER_IN, heightPt / PT_PER_IN, KEY_ENTRY_WIDTH_BW_IN, KEY_ENTRY_HEIGHT_BW_IN, orientation, maxPerLine);
  const entryWidthPt = entryWidthIn * PT_PER_IN;
  const entryHeightPt = entryHeightIn * PT_PER_IN;
...
function drawPlacedElement(page, { id, rect, elConfig, state, palette, fonts, textColor, blackWhiteEdition = false }, geom) {
  ...
  if (id === "colorKey") {
    drawKeyEntries(page, rectPt, palette, fonts.colorKey, textColor, blackWhiteEdition, state.colorKeyOrientation, align, state.colorKeyEntriesPerLine);
    return;
  }
...
export async function exportInteriorPdf(state, { onProgress } = {}) {
  ...
  for (const item of state.batchItems) {
    ...
    const sourceCanvas = await resolveItemSource(item, effective.sourceSmoothing, effective.sourceResolution);
    ...
    const renderOpts = {
      ...
      numberTintPercent: state.numberTintPercent,
      ...
      frameMarginCells: state.gridFrameMarginCells,
      blankColorIds: state.blankColorIds,
...
async function renderActiveItemFullPage(state, mode) {
  ...
  const effective = activeItem
    ...
    : {
        ...
        cornerTrimCorners: state.gridCornerTrimCorners,
  ...
  const sourceCanvas = activeItem ? await resolveItemSource(activeItem, effective.sourceSmoothing, effective.sourceResolution) : getPlaceholderSource();
